...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def show_realtime_tasks(self):
        ...
        for task in self.realtime_tasks:
            ...
            fr_time = datetime.datetime.strptime(task["start_time"], "%H:%M:%S").timestamp()
            to_time = datetime.datetime.strptime(task["end_time"], "%H:%M:%S").timestamp()
            if task['bus_id'] in dct:
                dct[task['bus_id']].append([task['id'], fr_time, to_time])
            else:
                dct[task['bus_id']] = [[task['id'], fr_time, to_time]]
        self.check_intersection(dct)
    ...
    def show_interval_tasks(self):
        ...
        for task in tasks:
            self.create_item(f"Task №{task['id']} | executor №{task['bus_id']} | status {task['status']} | time: {task.get('start_time')}-{task.get('end_time')}", ItemType.TASK, task)
    ...
    def open_item_info(self, item):
        ...
        if item_type == ItemType.TASK:
            title = f"Task {new_data.get('id')}"
        else:
            title = f"Bus {new_data.get('id')}"
        self.now_item_info.setWindowTitle(title)

    def check_intersection(self, dct: dict):
        for bus in dct:
            for i in range(len(dct[bus])):
                for j in range(i + 1, len(dct[bus])):
                    if (dct[bus][i][1] < dct[bus][j][2] and dct[bus][i][1] > dct[bus][j][1]) or \
                            (dct[bus][i][2] > dct[bus][j][1] and dct[bus][i][2] < dct[bus][j][2]):
                        err = QtWidgets.QMessageBox()

                        err.setText(f"Обнаружено пересечение у задач {dct[bus][i][0]} и {dct[bus][i][0]} для автобуса {bus}!")
                        err.show()
                        err.exec()
